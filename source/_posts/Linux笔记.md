...
在虚拟机的控制台输入：ifconfig
![image-20240919230036345](https://s2.loli.net/2025/03/06/U6kwCmile57QSEJ.png)

**将inet后的IP复制到FinalShell主机位置并填写虚拟机的用户名和密码**

![image-20240919225914185](https://s2.loli.net/2025/03/06/X2jRFK4tfNvAa3d.png)
...
要使用wq命令，您需要首先进入编辑器。在Linux中，比较常用的编辑器是Vi和Vim。以下是使用wq命令保存文件的步骤：
...
2. 在编辑模式下，按下i键以进入插入模式。现在您可以编辑文件。

3. 在编辑完成后，按下Esc键以退出插入模式。

4. 输入冒号(:)进入命令模式。

5. 输入wq并按下Enter键。这将保存文件并退出编辑器。

在Vi和Vim中，还有其他类似的保存和退出命令可以使用。例如，如果您只想保存文件而不退出编辑器，可以使用命令:w。如果只想退出而不保存更改，可以使用命令:q。
